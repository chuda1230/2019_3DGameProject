...
void CObjectsShader::BuildObjects(ID3D12Device *pd3dDevice, ID3D12GraphicsCommandList *pd3dCommandList) {
	...
	pWallMesh = new CWallMesh(pd3dDevice, pd3dCommandList);
	...
	for (int x = -xObjects; x <= xObjects; x++) {
		for (int y = -yObjects; y <= yObjects; y++) {
			for (int z = -zObjects; z <= zObjects; z++) {
				...
				//각 정육면체 객체의 위치를 설정한다.
				int random = rand()+10;
				pRotatingObject->SetPosition(fxPitch*x+random, fyPitch*y+random, fzPitch*z+random);
	...
	TestObject->SetMesh(pCubeMesh);
	TestObject->SetPosition(0,0,0);
	m_ppObjects[i++] = TestObject;
...
CRailShader::CRailShader() {}
void CRailShader::AnimateObjects(float fTimeElapsed)
{
	test++;
	auto RailObject = new CWallsObject;
	RailObject->SetMesh(pWallMesh);
	if (test % 35 == 0) {
		RailObject->SetPosition(FlatPos.x,FlatPos.y,FlatPos.z);
		RailObject->RotateTrace(FlatRight.x, FlatRight.y, FlatRight.z , FlatUp.x,FlatUp.y, FlatUp.z,FlatLook.x, FlatLook.y,FlatLook.z);
		m_vRails.push_back(RailObject);
	}
	if (m_vRails.size() > 20)
	{
		auto iter = m_vRails.begin();
		m_vRails.erase(iter);
	}
	for (int j = 0; j < m_vRails.size(); j++)
	{
		m_vRails[j]->Animate(fTimeElapsed);
	}
}
void CRailShader::ReleaseObjects()
{
	if (!m_vRails.empty())
	{
		for (int j = 0; j < m_vRails.size(); j++)
		{
			if (m_vRails[j]) delete m_vRails[j];
		}
	}
}
void CRailShader::ReleaseUploadBuffers()
{
	if (!m_vRails.empty())
	{
		for (int j = 0; j < m_vRails.size(); j++)
			m_vRails[j]->ReleaseUploadBuffers();
	}
}
void CRailShader::Render(ID3D12GraphicsCommandList *pd3dCommandList, CCamera *pCamera)
{
	CShader::Render(pd3dCommandList, pCamera);
	for (int j = 0; j < m_vRails.size(); j++)
	{
		if (!m_vRails.empty())
		{
			m_vRails[j]->Render(pd3dCommandList, pCamera);
		}
	}
}
CRailShader :: ~CRailShader() {}